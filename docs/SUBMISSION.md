...
|---|---|---|---|---|
| Technical execution | 30% | OpenCV 5 `Features` used substantively — ALIKED + LightGlue align each capture to the stored baseline of the same asset; CI on arm64 runners checks the runtime is really OpenCV 5, lints, type checks and holds a 90% coverage floor | `services/perception/alignment.py`, `services/perception/tests/`, `.github/workflows/ci.yml`, video 1:29–3:00 (`inlier_ratio 0.9988` on screen) | done |
| Innovation | 20% | **Longitudinal memory.** No previous winner kept state across inspections of the same asset; every prior project analyses one frame or one session. A second inspection retrieves the baseline from memory, aligns to it and locates the new defect. The trace of every run is hash-chained, and `GET /traces/{run_id}` reports the event where an edit broke it | `services/memory/`, `services/memory/tests/test_longitudinal.py`, `services/observability/trace.py`, [`docs/TECHNICAL_REPORT.md` §3](TECHNICAL_REPORT.md#3-what-makes-this-different-longitudinal-memory), video 1:29–3:00 | done |
