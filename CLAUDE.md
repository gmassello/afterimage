...
- `services/memory/` — one DynamoDB table (`pk=ASSET#id`, `sk` in `META` / `INSPECTION#ts#id` / `BASELINE#ts`) returning an asset's whole history in one query; the `META` item also carries the last inspection's capture key, timestamp, label and branch, so the gallery on `/` costs one scan and no per-asset query; images in S3 under `assets/{asset_id}/{inspection_id}/{name}.png`; baselines superseded, never overwritten. `runs.py` reads/writes run artefacts to the local FS or to S3 under `runs/` when `AFTERIMAGE_RUNS_S3=1`.
